...
Shader "Custom/Water11" 
{
	Properties 
	{
		_NrmMap2D ("NrmMap", 2D) = "white" {}
		_RefrTexture2D ("RefractionMap", 2D) = "white" {}
		...
		_SkyColor("SkyColor", Vector) = (0.0,0.0,0.0,1.0)
	}
	SubShader
	{
		Pass
		{
			...
			float4 FragmentShader(vertOut In) : SV_Target
			{
				float3 fog_color = float3(1.0f,1.0f,1.0f);
					...
					float4 vSkyColor = UNITY_SAMPLE_TEXCUBE(unity_SpecCube0, worldRefl) * (1.0f - _SkyColor.a) + _SkyColor * _SkyColor.a;
					//float4(172.0/255, 204.0/255, 250.0/255, 1.0);
					...
					float2 refr_tc = screenPos.xy - screenPos.z * vNormal3.xz * 0.5;
					float4 local_refr = tex2D(_RefrTexture2D, refr_tc);

					sea_clr = lerp(sea_clr.rgb,local_refr.rgb,local_refr.a);

					vSkyColor.rgb = vSkyColor.rgb + fSpec * vSunColor * 20.0f;
