...
Shader "Custom/Water9" 
{
	Properties 
	{
		_NrmMap2D ("NrmMap", 2D) = "white" {}
		//_RefTexture2D ("ReflectionMap", 2D) = "white" {}
		...
		Pass
		{
			...
			float4 FragmentShader(vertOut In) : SV_Target
			{
				float3 fog_color = float3(1.0f,1.0f,1.0f);
					...
					//float3 vSunColor = float3(0.89f, 0.73f, 0.56f);
					...
					//vSkyColor = float4(172.0/255, 204.0/255, 250.0/255, 1.0);
					...
					//float4 local_refl = tex2D(_RefTexture2D, refl_tc);
