		...
		Pass
		{
			...
			float4 FragmentShader(vertOut In) : SV_Target
			{
				float3 fog_color = float3(1.0f,1.0f,1.0f);
					...
					float4 local_refr = tex2D(_RefrTexture2D, refl_tc);
				...
				attenuation = max(0.7f, LIGHT_ATTENUATION(In));
